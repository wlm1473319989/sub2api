...
- 已落地：结算单退款主流程、2 分钟预览单、提交即冻结 `suspended`、退款申请列表/详情、管理端网关处理/人工凭证/完成/取消能力。
- 已落地：用户订阅页退款入口、用户/管理端退款申请页面、管理端新主路径 `/api/v1/admin/subscription-refund-requests/:id/*`，同时保留旧 `/admin/subscriptions/refund-requests/:id/*` 兼容。
- 本轮已补：旧用户订单退款入口在订阅订单场景下不再直接走 legacy refund，而是先校验“该订单必须是当前有效结算头来源订单”，然后复用结算单退款预览与提交能力。
- 本轮已补：旧用户/管理端订单退款兼容入口在订阅订单场景下，`refund-preview` 会返回 `preview_id`、`preview_token`、人工补退信息与订单分摊；`refund-request` / `refund` 必须基于该预览提交，不能再由后端“现算现提”绕过 2 分钟时效。
- 本轮已补：用户订单页、管理员订单页的订阅订单退款按钮已收口，不再继续引导走旧订单金额退款交互。
- 本轮已补：结算单退款预览响应补充“当前套餐、订阅到期时间、当前结算单 ID、结算后权益价值、当前剩余刀数、单位刀成本、退款模式、订单支付方式/通道信息、预览倒计时秒数”等字段；用户退款弹窗同步展示，满足“预览必须可核对”的要求。
- 本轮已补：用户/管理端退款申请详情页在 `subscription.status = suspended` 且退款申请处于处理中状态时，前端文案改为“退款处理中（已冻结）”，不再直接显示通用“已冻结”。
- 本轮已补：用户/管理端订阅列表接口会标记 `refund_freeze_active` 与当前活跃退款申请状态；订阅列表页对退款冻结中的 `suspended` 订阅统一显示“退款处理中（已冻结）”。
- 本轮已补：管理端旧 `AdminRefundDialog` 对订阅订单增加了组件级硬保护；即便误被打开，也不会再走旧金额退款预览/提交，而是只显示“请改走结算单退款申请列表”引导。
- 本轮已补：人工补退收款备注 `manual_transfer.remark` / `manual_receiver_remark` 已打通到用户提交接口、旧管理端兼容入口、退款申请 DTO、用户/管理端详情页展示，以及前端类型和测试桩。
- 本轮已补：旧用户订单退款兼容入口提交订阅退款时，`manual_transfer.remark` 也已透传到结算单退款服务，不再出现“订阅主入口有备注、旧订单入口丢备注”的链路不一致。
- 本轮已补：当同一订阅已存在 `submitted/gateway_processing/manual_pending/failed` 退款申请时，创建新预览会把数据库唯一约束冲突明确映射为业务错误 `SETTLEMENT_REFUND_ALREADY_PENDING`，前端文案也已补齐。
- 本轮已补：退款提交阶段的 `previewed -> submitted` 条件更新已补上 `preview_expires_at >= now`，与计划书要求的 2 分钟预览时效条件一致。
- 本轮已补：退款提交、完成、取消阶段已在事务内显式锁定订阅；提交和完成还会锁定当前 effective settlement head，补强与续费、升级等流程并发时的状态保护，贴合计划书的“提交幂等与锁”要求。
- 本轮已补：退款提交、网关处理、人工凭证上传、完成、取消等关键状态迁移已补结构化审计事件，使用现有 `OpsSystemLogSink` / `component = audit.subscription_refund` 进入系统日志索引，不再完全缺失审计留痕。
- 本轮已确认：退款图片暂按字符串方案落地，不引入新的上传组件或独立图片服务。用户收款码 `manual_receiver_qr_image_url` 与管理员凭证 `manual_transfer_proof_url` 继续直接存入 MySQL `TEXT` 字段，可存可访问图片 URL，前端也按字符串输入。
- 本轮已补：字符串图片方案增加最小输入约束。后端会校验图片引用若带 scheme 则必须为合法 `http(s)` URL；无 scheme 的字符串继续允许作为站内已存储路径，兼容当前 MySQL 字符串过渡方案。
- 仍待补强：管理员从旧订单退款入口直接完成订阅退款时，目前已切到结算单退款闭环，但人工补差场景仍建议以后续退款申请详情页为主操作面，避免在旧订单弹窗里继续承载复杂状态。
- 仍待补强：当前审计留痕已通过系统日志索引落地，但尚未建设 `subscription_refund_requests` 维度的专用审计表；如果后续需要更强的对账、筛选和长期归档能力，建议再补独立审计存储。
- 仍待补强：Go 侧 `settlement_refund_request_store` 相关 SQL mock 测试已补 `manual_receiver_remark` 列和关键断言，但仍需在可用 Go 工具链环境下实际跑通后端测试确认没有遗漏扫描位序问题。
- 仍待补强：Go 侧自动化校验当前受本机 Go 依赖/工具链环境影响，前端校验已通过，Go 全量测试需在可用的 Go 标准库/模块代理环境下复跑。
...
支付网关退款必须封顶，不能超过订单剩余可退实付金额。结算单退款场景下，网关退款金额还必须按币种最小单位向下取整，不能因为四舍五入产生超退；向下取整产生的差额必须回流到 `manual_transfer_amount`。普通订单退款的比例换算逻辑不受这条规则影响。
...
当前项目没有明确的通用附件表。结合当前实现决策，v1 暂按最小字符串方案执行：

- 不引入新的图片服务、附件表、对象存储接入或专用上传组件。
- `subscription_refund_requests.manual_receiver_qr_image_url`、`subscription_refund_requests.manual_transfer_proof_url` 继续使用 MySQL `TEXT` 字段存储字符串。
- 当前约定字符串内容优先为可访问图片 URL；如后续要升级到受控文件 key 或对象存储，再在该字段上兼容扩展。
- 为了避免写入明显非法值，服务端当前会做最小校验：若字符串带 scheme，则必须是合法 `http(s)` URL；不带 scheme 时可继续作为历史/站内存储路径保存。
- `manual_transfer_proof_uploaded_at`、`manual_transfer_operator_user_id` 仍照常记录上传时间与操作管理员。
- 后续若要补强安全性，再单独引入受控上传、MIME/大小校验和鉴权访问链路。
