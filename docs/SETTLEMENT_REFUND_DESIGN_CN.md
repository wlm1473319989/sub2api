...
1. 新增退款申请和退款分摊数据模型及迁移。
2. 抽出 `SettlementRefundService`，实现残值预览、订单分摊、2 分钟预览校验。
3. 实现用户提交申请和订阅 `suspended` 冻结。
4. 实现管理端网关退款处理、人工补退凭证上传、完成退款。
5. 接入用户侧和管理端前端页面。
6. 将订阅订单旧退款入口收敛到结算单退款服务。
7. 补充单元测试、集成测试和前端交互测试。
