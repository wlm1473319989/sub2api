            ...
            <template v-if="selectedPlan">
              <div class="card p-5">
                ...
                <div class="flex items-baseline gap-2">
                  ...
                  <span class="text-3xl font-bold text-gray-900 dark:text-white">{{ formatSelectedSubscriptionPaymentAmount(selectedPlanOrderAmount) }}</span>
                ...
                <div class="mt-3 grid grid-cols-2 gap-3">
                  ...
                  <div v-if="selectedPlan.weekly_quota_knives != null">
                    <span class="text-xs text-gray-400 dark:text-gray-500">{{ t('payment.planCard.weeklyLimit') }}</span>
                    <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ selectedPlan.weekly_quota_knives }}</div>
                  </div>
                  <div v-if="selectedPlan.monthly_quota_knives != null">
                    <span class="text-xs text-gray-400 dark:text-gray-500">{{ t('payment.planCard.monthlyLimit') }}</span>
                    <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ selectedPlan.monthly_quota_knives }}</div>
                  </div>
                  <div v-if="selectedPlan.daily_quota_knives == null && selectedPlan.weekly_quota_knives == null && selectedPlan.monthly_quota_knives == null">
              ...
              <button :class="['btn w-full py-3 text-base font-medium', paymentButtonClass]" :disabled="!canSubmitSubscription || submitting" @click="confirmSubscribe">
                ...
                <span v-else>{{ t(selectedPlanAction === 'upgrade' ? 'payment.upgradeNow' : selectedPlanAction === 'renew' ? 'payment.renewNow' : 'payment.subscribeNow') }} {{ formatSelectedPaymentAmount(subTotalAmount) }}</span>
              </button>
...
const subPaymentAmount = computed(() => {
  return subscriptionPaymentAmountForCurrency(selectedPlanOrderAmount.value, selectedCurrency.value)
})
