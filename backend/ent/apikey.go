...
type APIKey struct {
	...
	// Whether this API key allows automatic failover to a paid backup group
	AllowPaidFailover bool `json:"allow_paid_failover,omitempty"`
	...
	for i := range columns {
		...
		case apikey.FieldAllowPaidFailover:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field allow_paid_failover", values[i])
			} else if value.Valid {
				_m.AllowPaidFailover = value.Bool
			}
		default:
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("allow_paid_failover=")
	builder.WriteString(fmt.Sprintf("%v", _m.AllowPaidFailover))
	builder.WriteByte(')')
