...
func (_q *PaymentOrderQuery) QuerySubscriptionRefundAllocations() *SubscriptionRefundAllocationQuery {
...
func (_q *PaymentOrderQuery) WithSubscriptionRefundAllocations(opts ...func(*SubscriptionRefundAllocationQuery)) *PaymentOrderQuery {
	query := (&SubscriptionRefundAllocationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	...
	if query := _q.withSubscriptionRefundAllocations; query != nil {
		if err := _q.loadSubscriptionRefundAllocations(ctx, query, nodes,
			func(n *PaymentOrder) { n.Edges.SubscriptionRefundAllocations = []*SubscriptionRefundAllocation{} },
...
func (_q *PaymentOrderQuery) loadSubscriptionRefundAllocations(ctx context.Context, query *SubscriptionRefundAllocationQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *SubscriptionRefundAllocation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptionrefundallocation.FieldPaymentOrderID)
	}
	query.Where(predicate.SubscriptionRefundAllocation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentorder.SubscriptionRefundAllocationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
