...
func TestUsageLogRepositoryCreateSyncRequestTypeAndLegacyFields(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO usage_logs").
		WithArgs(
			...
			sqlmock.AnyArg(), // origin_group_id
			sqlmock.AnyArg(), // routed_group_id
			sqlmock.AnyArg(), // failover_reason
			...
			log.BalanceCost,
			log.RateMultiplier,
			log.SubscriptionRateMultiplier,
			log.BalanceRateMultiplier,
...
func TestPrepareUsageLogInsert_PersistsSplitBillingCosts(t *testing.T) {
	...
	require.Equal(t, 1.2, prepared.args[25])
	require.Equal(t, 0.7, prepared.args[26])
	require.Equal(t, 0.5, prepared.args[27])
}
...
func TestUsageLogRepositoryGetStatsWithFiltersRequestTypePriority(t *testing.T) {
	...
	mock.ExpectQuery("FROM usage_logs\\s+WHERE \\(request_type = \\$1 OR \\(request_type = 0 AND stream = FALSE AND openai_ws_mode = FALSE\\)\\)").
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"total_account_cost",
			"avg_duration_ms",
		}).AddRow(int64(1), int64(2), int64(3), int64(4), 1.2, 1.0, 0.7, 0.3, 1.2, 20.0))
	...
	require.Equal(t, 0.7, stats.TotalSubscriptionCost)
	require.Equal(t, 0.3, stats.TotalBalanceCost)
...
func TestUsageLogRepositoryGetStatsWithFiltersAlwaysReturnsAccountCost(t *testing.T) {
	...
	mock.ExpectQuery("FROM usage_logs").
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"total_subscription_cost", "total_balance_cost",
			"total_account_cost", "avg_duration_ms",
		}).AddRow(int64(50), int64(1000), int64(2000), int64(100), 15.0, 12.5, 8.0, 4.5, 11.0, 100.0))
	...
	require.Equal(t, 8.0, stats.TotalSubscriptionCost)
	require.Equal(t, 4.5, stats.TotalBalanceCost)
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	...
	t.Run("request_type_ws_v2_overrides_legacy", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullInt64{},   // origin_group_id
			sql.NullInt64{},   // routed_group_id
			sql.NullString{},  // failover_reason
			...
			0.9,               // balance_cost
			1.0,               // rate_multiplier
			1.0,               // subscription_rate_multiplier
			1.0,               // balance_rate_multiplier
