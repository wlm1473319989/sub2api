...
func (s *stubAdminService) AdminUpdateAPIKeyAllowPaidFailover(ctx context.Context, keyID int64, allow bool) (*service.APIKey, error) {
	for i := range s.apiKeys {
		if s.apiKeys[i].ID == keyID {
			s.apiKeys[i].AllowPaidFailover = allow
			k := s.apiKeys[i]
			return &k, nil
		}
	}
