...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	// 2. Re-check billing
	resolvedSubscription, err := h.billingCacheService.CheckBillingEligibility(requestCtx, apiKey.User, apiKey, apiKey.Group, subscription, service.QuotaPlatform(requestCtx, apiKey))
	if err != nil {
