...
func TestCalculateStatsCost_TokenBilling_WithCacheTTLPricesWithoutBreakdown(t *testing.T) {
	pricing := &ChannelModelPricing{
		BillingMode:       BillingModeToken,
		CacheWritePrice:   testPtrFloat64(0.003),
		CacheWrite1hPrice: testPtrFloat64(0.005),
	...
	result := calculateStatsCost(pricing, tokens, 1)
	require.NotNil(t, result)
	// Aggregate-only upstream responses use the backwards-compatible 5m price.
