...
func resolveSubscriptionRemainingQuota(sub *UserSubscription) float64 {
	if sub == nil {
		return 0
	}

	dailyQuota := sub.DailyQuotaKnives
	weeklyQuota := sub.WeeklyQuotaKnives
	monthlyQuota := sub.MonthlyQuotaKnives
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	// Token billing applies the peak factor; image-per-request billing stays on the base/image multiplier.
	// Subscription billing keeps the local group subscription multiplier semantics.
