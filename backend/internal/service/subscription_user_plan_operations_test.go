...
func TestPurchaseNewPlanCreatesSnapshotSubscription(t *testing.T) {
	...
	require.Equal(t, 0, sub.StartsAt.Hour())
	require.Equal(t, 0, sub.StartsAt.Minute())
	require.Equal(t, 0, sub.StartsAt.Second())
	require.Equal(t, 0, sub.StartsAt.Nanosecond())
...
func TestRenewActivePlanExtendsWithoutResettingUsage(t *testing.T) {
	...
	require.Equal(t, before.ExpiresAt.AddDate(0, 0, 30), renewed.ExpiresAt)
