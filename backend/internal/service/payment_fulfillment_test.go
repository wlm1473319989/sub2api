...
func TestExecuteSubscriptionFulfillmentAppliesAffiliateRebate(t *testing.T) {
	...
	user := h.createUser(t, "subscription-affiliate@example.com")
	group := h.createGroup(t, "subscription-affiliate-group", nil, nil, nil)
	plan := h.createPlan(t, "Subscription Affiliate", 120, 30, "day", &group.ID, nil, nil, nil)
	...
	h.paymentSvc.affiliateService = NewAffiliateService(affiliateRepo, settingSvc, nil, nil)

	err := h.paymentSvc.ExecuteSubscriptionFulfillment(ctx, order.ID)
	require.NoError(t, err)
	...
	require.InDelta(t, 24.0, affiliateRepo.accrueCalls[0].amount, 0.00000001)
	...
	require.Equal(t, 1, h.userSubRepo.createCalls)
...
func TestExecuteSubscriptionFulfillmentDoesNotDuplicateWorkAfterLegacySuccessAudit(t *testing.T) {
	...
	user := h.createUser(t, "subscription-affiliate-idempotent@example.com")
	group := h.createGroup(t, "subscription-affiliate-idempotent-group", nil, nil, nil)
	plan := h.createPlan(t, "Subscription Affiliate Idempotent", 80, 30, "day", &group.ID, nil, nil, nil)
	order := h.createSubscriptionOrder(t, user, plan, subscriptionActionPurchase, OrderStatusPaid, time.Now())

	_, err := client.PaymentAuditLog.Create().
		SetOrderID(strconv.FormatInt(order.ID, 10)).
		SetAction("SUBSCRIPTION_SUCCESS").
		SetDetail(`{"planID":100,"action":"purchase"}`).
		SetOperator("system").
		Save(ctx)
	require.NoError(t, err)
	_, err = client.PaymentAuditLog.Create().
	...
	require.Zero(t, h.userSubRepo.createCalls)
}
