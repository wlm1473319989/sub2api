...
func TestGetModelPricing_OpenAICompactAliasesFallback(t *testing.T) {
	...
	tests := []struct {
		...
		outputPrice        float64
		cacheWrite         float64
		priorityCacheWrite float64
		cacheRead          float64
		longContext        int
	}{
		{model: "gpt5.5", inputPrice: 2.5e-6, outputPrice: 15e-6, cacheWrite: 2.5e-6, priorityCacheWrite: 0, cacheRead: 0.25e-6, longContext: 272000},
		{model: "gpt5.6-sol", inputPrice: 5e-6, outputPrice: 30e-6, cacheWrite: 6.25e-6, priorityCacheWrite: 12.5e-6, cacheRead: 0.5e-6, longContext: 272000},
		{model: "openai/gpt-5.6-terra", inputPrice: 2.5e-6, outputPrice: 15e-6, cacheWrite: 3.125e-6, priorityCacheWrite: 6.25e-6, cacheRead: 0.25e-6, longContext: 272000},
		{model: "gpt-5.6-luna-openai-compact", inputPrice: 1e-6, outputPrice: 6e-6, cacheWrite: 1.25e-6, priorityCacheWrite: 2.5e-6, cacheRead: 0.1e-6, longContext: 272000},
		{model: "openai/gpt5.4", inputPrice: 2.5e-6, outputPrice: 15e-6, cacheWrite: 2.5e-6, priorityCacheWrite: 0, cacheRead: 0.25e-6, longContext: 272000},
		{model: "gpt5.4-mini", inputPrice: 7.5e-7, outputPrice: 4.5e-6, cacheWrite: 0, priorityCacheWrite: 0, cacheRead: 7.5e-8, longContext: 0},
		{model: "gpt5.3codexspark", inputPrice: 1.5e-6, outputPrice: 12e-6, cacheWrite: 1.5e-6, priorityCacheWrite: 0, cacheRead: 0.15e-6, longContext: 0},
	}
	...
	for _, tt := range tests {
		t.Run(tt.model, func(t *testing.T) {
			...
			require.InDelta(t, tt.priorityCacheWrite, pricing.CacheCreationPricePerTokenPriority, 1e-12)
...
func TestCalculateCostWithServiceTier_Gpt56PriorityUsesOfficialCacheWrite(t *testing.T) {
	svc := newTestBillingService()
	tokens := UsageTokens{CacheCreationTokens: 1_000_000}

	cost, err := svc.CalculateCostWithServiceTier("gpt-5.6-sol", tokens, 1.0, "priority")
	require.NoError(t, err)
	require.InDelta(t, 12.5, cost.CacheCreationCost, 1e-10)
	require.InDelta(t, 12.5, cost.TotalCost, 1e-10)
}
...
func TestBillingServiceGetModelPricing_Gpt56OverridesStaleDynamicPricing(t *testing.T) {
	pricingSvc := &PricingService{
		pricingData: map[string]*LiteLLMModelPricing{
			"gpt-5.6-terra": {
	...
	require.InDelta(t, 6.25e-6, terra.CacheCreationPricePerTokenPriority, 1e-12)
	...
	require.InDelta(t, 2.5e-6, luna.CacheCreationPricePerTokenPriority, 1e-12)
