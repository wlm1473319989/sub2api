...
const apiKeyAuthSnapshotVersion = 15 // v15: include group peak rate, OpenAI backup failover, and allow_paid_failover fields
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		AllowPaidFailover: apiKey.AllowPaidFailover,
	...
	if apiKey.Group != nil {
		snapshot.Group = &APIKeyAuthGroupSnapshot{
			...
			BackupFailoverEnabled:           apiKey.Group.BackupFailoverEnabled,
			BackupGroupID:                   apiKey.Group.BackupGroupID,
			BackupFailoverConfig:            apiKey.Group.BackupFailoverConfig,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		AllowPaidFailover: snapshot.AllowPaidFailover,
		User: &User{
			ID:                         snapshot.User.ID,
	...
	if snapshot.Group != nil {
		apiKey.Group = &Group{
			...
			BackupFailoverEnabled:           snapshot.Group.BackupFailoverEnabled,
			BackupGroupID:                   snapshot.Group.BackupGroupID,
			BackupFailoverConfig:            snapshot.Group.BackupFailoverConfig,
